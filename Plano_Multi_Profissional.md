...
| **2b — Lumi (n8n)** ✅ | Script `n8n/scripts/multiprof-add-lista-profissionais.js` (idempotente): node "Lista Profissionais" (httpRequestTool → `GET {BRIDGE_URL}/profissionais`) + `ai_tool`, `profissionalId` em Verifica/Cria/Remarcar, seção nova no systemMessage (8→9), sticky. **DEV `yFSw0JMMD93EGZMa` + Standalone `9PsUjET74L2NblWv`: aplicados** (multi-prof + regra de apresentação de horários reescrita — ver commit da apresentação). **Standalone testado ao vivo (execuções n8n 5179/5183, 10/09):** `Lista Profissionais` OK, roteou "canal" → Dr. Rafael pelo nome, `verificar_disponibilidade` com o `profissionalId` real dele, slots da agenda própria dele. A apresentação dos horários ficou ruim nessa 1ª execução (lista longa, agrupada) → daí a reescrita da regra; **re-testar no demo.** `server.js` raiz **redeployado** — `GET /profissionais` no ar devolvendo `[Dra. Aline]`. **PROD `K2xRqOwS0N0AcoqG`: APLICADO 10/09** (os 2 scripts, PUT+activate, `draft==ativo`, 126→128 nós). versionId novo `9d3bd76d-3d95-4857-841e-30a79b9be74f`, **anterior (revert) `a018645d-03e0-4627-82cb-984dffff6f23`**. Bridge de PROD `/profissionais` devolve `[Dra. Aline]` → guarda do prompt → inerte pra ela. Fase 2b concluída nos 3 workflows.
_À parte (não é multi-prof), FEITO:_ **cadastro leve pro standalone** — toggle `_modulo_cadastro_completo` no template + compilador; no modo leve o standalone só pede o nome (adulto) ou nome+nascimento+responsável (dependente menor), sem CPF/CEP/e-mail. Harness validou (paciente novo adulto: nome → horário → valor → agenda, zero interrogatório). `n8n/scripts/fix-cadastro-leve.js` aplicado no **Standalone**. Aline PROD mantém cadastro completo. **Dep.:** DEV/PROD só retornam dados na tool depois do redeploy do `server.js` raiz (o `GET /profissionais` está commitado mas não deployado) — enquanto isso a tool dá 404 e a guarda do prompt trata como "clínica de 1", inerte/seguro. | execução n8n + demo |
