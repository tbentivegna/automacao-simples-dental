...
_Pendente à parte (não é multi-prof):_ o demo pede cadastro completo demais (CPF/CEP/e-mail) — o standalone-bridge só precisa de nome (+ nascimento/responsável se for dependente). Reescrever pro standalone via toggle `_modulo_cadastro_completo` no template. **Dep.:** DEV/PROD só retornam dados na tool depois do redeploy do `server.js` raiz (o `GET /profissionais` está commitado mas não deployado) — enquanto isso a tool dá 404 e a guarda do prompt trata como "clínica de 1", inerte/seguro. | execução n8n + demo |
