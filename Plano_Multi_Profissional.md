...
| **2b — Lumi (n8n)** ⏳ | Script `n8n/scripts/multiprof-add-lista-profissionais.js` (idempotente): node "Lista Profissionais" (httpRequestTool → `GET {BRIDGE_URL}/profissionais`) + `ai_tool`, `profissionalId` em Verifica/Cria/Remarcar, seção nova no systemMessage (8→9), sticky. **DEV `yFSw0JMMD93EGZMa` (inativo): aplicado + verificado** (118→120 nós). **Standalone `9PsUjET74L2NblWv` (ativo): aplicado + republicado** (`draft==ativo`, 112→114 nós); o standalone-bridge `/profissionais` responde ao vivo com os 3 do demo. **Falta:** teste funcional do Standalone (mandar uma mensagem de verdade no WhatsApp do demo, ex. "preciso tratar um canal", e conferir a execução no n8n — o demo não tem tráfego desde 04/09) e **PROD `K2xRqOwS0N0AcoqG`** (por último; `draft==ativo` hoje, o script já faz PUT+activate+verifica). **Dep.:** DEV/PROD só retornam dados na tool depois do redeploy do `server.js` raiz (o `GET /profissionais` está commitado mas não deployado) — enquanto isso a tool dá 404 e a guarda do prompt trata como "clínica de 1", inerte/seguro. | execução n8n + demo |
