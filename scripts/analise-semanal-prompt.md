...
- **Limitação confirmada do `n8n_chat_histories` (achado id=4, 03/09/2026)**: quando a Lumi chama MAIS DE UMA tool na mesma resposta (ex: cria o agendamento e, na sequência, registra o consentimento de lembrete), o histórico salvo às vezes perde a(s) chamada(s) anteriores à última -- confirmado com evidência real (execução n8n 4105, DEV: `Cria_Agendamento` retornou sucesso, mas só `Registrar_Consentimento_Lembrete` + a resposta final ficaram gravados no histórico; a chamada de `Cria_Agendamento` simplesmente não aparece). É um comportamento do node nativo "Postgres Chat Memory" (`@n8n/n8n-nodes-langchain.memoryPostgresChat`), fora do nosso controle direto (não é uma query nossa pra corrigir). **Por isso: nunca conclua "a Lumi não chamou a tool X" só pela ausência dela no `n8n_chat_histories`, especialmente num turno onde o texto final sugere que mais de uma ação aconteceu (ex: menciona agendamento E lembrete).** Se isso for central pra uma classificação (ex: parece que ela pulou uma etapa), confirme cruzando com a execução real do n8n (`GET /api/v1/executions?workflowId=...` + `includeData=true`, campo `data.resultData.runData['AI Agent'][0].data.main[0][0].json.intermediateSteps`) antes de registrar como correção real.
