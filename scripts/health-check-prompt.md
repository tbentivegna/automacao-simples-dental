...
- **Postgres** (compartilhado entre tudo): connection string em `.env` na raiz (`DATABASE_URL`). Tabelas relevantes: `n8n_chat_histories` (todo o histórico de mensagens trocadas), `agent_actions` (pendências que a Lumi encaminhou pra equipe), `funil_agendamento` (tentativas de agendamento e resgates), `cliente` (cadastro de pacientes), `eventos_agenda` (analytics de agendamentos).
  - **Limitação conhecida (achado id=4, 03/09/2026)**: `n8n_chat_histories` pode perder a chamada de uma tool quando a Lumi chama mais de uma na mesma resposta (só a última + a resposta final ficam gravadas -- confirmado via execução real do n8n). Ausência de uma tool call nesse histórico não é prova de que ela não aconteceu; se isso for crítico pra uma conclusão, confirme cruzando com a execução real do n8n antes de reportar como bug.
